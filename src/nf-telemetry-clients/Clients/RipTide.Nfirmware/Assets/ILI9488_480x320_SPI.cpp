...
void DisplayDriver::Clear()
{
    ...
    static const uint32_t totalPixels = Attributes.Width * Attributes.Height * 3 / 2;
    g_DisplayInterface.FillData16(0, totalPixels);
}
...
{
    CLR_UINT16 xOffset = g_DisplayInterfaceConfig.Screen.x;
    CLR_UINT16 yOffset = g_DisplayInterfaceConfig.Screen.y;
    ...
    CLR_UINT32 data[])
{
    ...
    // Precompute shifts for RGB scaling
    ...
    {
        uint32_t rowOffset = y * Attributes.Width; // Precompute row offset
...
inline CLR_UINT32 DisplayDriver::PixelsPerWord()
{
    static const uint32_t pixelsPerWord = 32 / Attributes.BitsPerPixel;
    return pixelsPerWord;
}

inline CLR_UINT32 DisplayDriver::WidthInWords()
{
    static const uint32_t widthInWords = (Attributes.Width + (PixelsPerWord() - 1)) / PixelsPerWord();
    return widthInWords;
}

inline CLR_UINT32 DisplayDriver::SizeInWords()
{
    static const uint32_t sizeInWords = WidthInWords() * Attributes.Height;
    return sizeInWords;
}

inline CLR_UINT32 DisplayDriver::SizeInBytes()
{
    static const uint32_t sizeInBytes = SizeInWords() * sizeof(CLR_UINT32);
    return sizeInBytes;
}
